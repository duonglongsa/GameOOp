...
public class Assets {
	...
	public static BufferedImage[] player_right, player_up, player_left, player_down, hurt_left, hurt_right, die_left,
			die_right;
	public static BufferedImage[] player2_right, player2_up, player2_left, player2_down, hurt2_left, hurt2_right,
			die2_left, die2_right;

	// attack
	public static BufferedImage[] attack_left, attack_right, extra_left, extra_right, skill;

	public static BufferedImage[] attack2_left, attack2_right, extra2_left, extra2_right, skill2;
...
//	public static BufferedImage[] button_play, button_settings, share, button_exit, github, facebook, google;
	...
	public static void init() {
		...
		// second character
		SpriteSheet sheet2 = new SpriteSheet(ImageLoader.loadImage("/player/Mage.png"));
		player2_right = new BufferedImage[6];
		for (int i = 0; i < 6; i++) {
			player2_right[i] = sheet2.crop(width * (i), 0, width, height);
		}

		player2_left = new BufferedImage[6];
		for (int i = 0; i < 6; i++) {
			player2_left[i] = sheet2.crop(width * (i), height, width, height);
		}

		player2_up = new BufferedImage[4];
		for (int i = 0; i < 4; i++) {
			player2_up[i] = sheet2.crop(width * (i), height * 2, width, height);
		}

		player2_down = new BufferedImage[4];
		for (int i = 0; i < 4; i++) {
			player2_down[i] = sheet2.crop(width * (i), height * 3, width, height);
		}

		// attack
		attack2_right = new BufferedImage[4];
		for (int i = 0; i < 4; i++) {
			attack2_right[i] = sheet2.crop(width * (i), height * 4, width, height);
		}
		extra2_right = new BufferedImage[10];
		for (int i = 0; i < 10; i++) {
			extra2_right[i] = sheet2.crop(width * (i), height * 4, width, height);
		}

		attack2_left = new BufferedImage[4];
		for (int i = 0; i < 4; i++) {
			attack2_left[i] = sheet2.crop(width * (i), height * 5, width, height);
		}
		extra2_left = new BufferedImage[10];
		for (int i = 0; i < 10; i++) {
			extra2_left[i] = sheet2.crop(width * (i), height * 5, width, height);
		}

		// hurt
		hurt2_right = new BufferedImage[4];
		for (int i = 0; i < 4; i++) {
			hurt2_right[i] = sheet2.crop(width * (i), height * 6, width, height);
		}

		hurt2_left = new BufferedImage[4];
		for (int i = 0; i < 4; i++) {
			hurt2_left[i] = sheet2.crop(width * (i), height * 7, width, height);
		}

		// die
		die2_right = new BufferedImage[10];
		for (int i = 0; i < 10; i++) {
			die2_right[i] = sheet2.crop(width * i, height * 8, width, height);
		}
		die2_left = new BufferedImage[10];
		for (int i = 0; i < 5; i++) {
			die2_left[i] = sheet2.crop(width * (i), height * 9, width, height);
		}
